...
model.load_weights('best_ep036-loss2.248-val_loss2.266.h5')
...
for i in range(len(test_lines)):
    ...
    boxes_, scores_, classes_ = yolo_eval(result1, anchors, num_classes, image_shape,
                                          score_threshold=0.3, iou_threshold=0.2)
    ...
    if len(boxes_1) > 0:
        for k in range(len(boxes_1)):
            ...
            cv2.putText(detect_img, text, (int((a1 + a2) / 2), int((b1 + b2) / 2)), 1, 1, (0, 0, 255))
